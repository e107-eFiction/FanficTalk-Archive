...
$chapid = isset($_GET['chapid']) && isNumber($_GET['chapid']) ? $_GET['chapid'] : 0;
	...
	$storyquery = dbquery("SELECT story.validated, story.catid, story.sid, story.title, story.summary, story.uid, "._PENNAMEFIELD." as penname, chapter.inorder, chapter.edited, story.coauthors FROM ".TABLEPREFIX."fanfiction_stories as story, ".TABLEPREFIX."fanfiction_chapters  as chapter, "._AUTHORTABLE." WHERE "._UIDFIELD." = story.uid AND chapter.sid = story.sid AND chapter.chapid ='$_GET[chapid]' LIMIT 1");

		list($storyvalid, $catid, $sid, $title, $summary, $authoruid, $author, $inorder, $coauthors, $edited) = dbrow($storyquery);
		...
		if(($edited == 0) || ($edited == 3)){
			...
			dbquery("UPDATE ".TABLEPREFIX."fanfiction_chapters SET validated = '-1', edited = '$edited', v_uid = '0' WHERE chapid = '$_GET[chapid]'");
