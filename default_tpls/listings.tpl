<!-- START BLOCK : listings -->
{seriesheader}
<!-- START BLOCK : seriesblock -->
<div class="listbox {oddeven}">
<div class="title">{title} by {author}</div>
<span class="label">Categories:</span> {category}<br />
<div class="content"><span class="label">Summary: </span>{summary}<br />
<span class="label">Parent Series:</span> {parentseries}<br />
<div class="gb-50">
	<span class label="characters"><b>Characters:</b> {characters}</span><br/>
	<hr class="style2">
	<span class label="pairings"><b>Pairings:</b> {pairings}</span><br/>
	<hr class="style2">
	<span class label="representation"><b>Representation:</b> {representation}</span><br/>
	<hr class="style2">
	<span class label="groups"><b>FFT Groups:</b> {groups}</span></br/>
</div>
<div class="gb-50">
	<span class label="advisories"><b>Rating and Advisories (required):</b> {rating} ({advisories})</span>
	<hr class="style2">
	<span class label="warnings"><b>Content Warnings (optional):</b> {warnings}</span><br/>
	<hr class="style2">
	<span class label="genre"><b>Genre:</b> {genre}</span><br/>
	<hr class="style2">
	<span class label="tropes"><b>Tropes:</b> {tropes}</span><br/>
	<hr class="style2">
	<span class label="type"><b>Story Type:</b> {type}</span><br/>
</div>
<div class="gb-full">
		<div align="center"><span class label="Series"><b>Series:</b> {serieslinks}</span></div>
		<div align="center">{addtofaves} {tracker} {last_read}<br/></div>
		<div align="center">{adminlinks}</div></div></div>
{comment}
<!-- END BLOCK : seriesblock -->
{stories}
<!-- START BLOCK : storyblock -->
<div class="listbox {oddeven}">
  <div class="gb-full"><br/>
  	<div id="pagetitle">{featuredstory} {title} by {author}</div>
  	<div id="sort">
  			<div align="center"><b>{category}</b></div>
  	</div>
  	<div id="output">
  		<div class="jumpmenu">{jumpmenu}</div>
  		<div class="listbox">
  			<br/>
  			<div align="center"><blockquote2>{summary}</blockquote2></div>
  	</div>
  	<br/>
  </div>
  <div class="gb-50">
  	<span class label="characters"><b>Characters:</b> {characters}</span><br/>
  	<hr class="style2">
  	<span class label="pairings"><b>Pairings:</b> {pairings}</span><br/>
  	<hr class="style2">
  	<span class label="representation"><b>Representation:</b> {representation}</span><br/>
  	<hr class="style2">
  	<span class label="type"><b>Story Type:</b> {type}</span><br/>
  	<hr class="style2">
  	<span class label="groups"><b>FFT Groups:</b> {groups}</span></br/>
  </div>
  <div class="gb-50">
  	<span class label="advisories"><b>Rating and Advisories (required):</b> {rating} ({advisories})</span><br/>
  	<hr class="style2">
  	<span class label="warnings"><b>Content Warnings (optional):</b> {warnings}</span><br/>
  	<hr class="style2">
  	<span class label="genre"><b>Genre:</b> {genre}</span><br/>
  	<hr class="style2">
  	<span class label="tropes"><b>Tropes:</b> {tropes}</span><br/></div></div>
  <div class="gb-full">
  		<div align="center">{completed} &middot; <b>Published:</b> {published} &middot; <b>Updated:</b> {updated} &middot; <b>Words:</b> {wordcount} &middot; <b>Chapters:</b> {numchapters} &middot; <b>Reviews:</b> {numreviews} &middot; <b>Likes:</b> {score} &middot; <b>Reads:</b> {count}</div><br/>
  		<div align="center"><span class label="Series"><b>Series:</b> {serieslinks}</span></div>
  		<div align="center">{addtofaves} {tracker} {last_read}<br/></div>
  		<div align="center">{adminlinks}</div>
      <br/>
      <hr class="style2">
</div></div>
{comment}
<!-- END BLOCK : storyblock -->
{pagelinks}
<!-- END BLOCK : listings -->
