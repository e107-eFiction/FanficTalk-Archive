...
while($series = dbassoc($seriesresults)) {
	...
	while($o = dbassoc($others)) {
		...
		else if(!empty($o['subseriesid'])) $sub = dbassoc(dbquery(_SERIESQUERY." AND seriesid = ".$o['seriesid']));
		if($o['inorder'] < $series['inorder']) {
			if($sibling) $thisSeries .= " ".sprintf(_SE_SERIES_PREVST, title_link($sibling));
			else if($sub) $thisSeries .= " ".sprintf(_SE_SERIES_PREVSE, "<a href='"._BASEDIR."viewseries.php?seriesid=".$sub['seriesid']."'>".stripslashes($sub['title'])."</a>");
		}
		else if($o['inorder'] > $series['inorder']) {
			if($sibling) $thisSeries .= " ".sprintf(_SE_SERIES_NEXTST, title_link($sibling));
			else if($sub) $thisSeries .= " ".sprintf(_SE_SERIES_NEXTSE, "<a href='"._BASEDIR."viewseries.php?seriesid=".$sub['seriesid']."'>".stripslashes($sub['title'])."</a>");
		}
	}
	$seSeries[] = $thisSeries;
}
