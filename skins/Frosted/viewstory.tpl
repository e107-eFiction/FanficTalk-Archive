<!-- INCLUDE BLOCK : header -->
<div class="gb-full">
	<h2>{title} by {author}</h2>
	<div id="jumpmenu">{jumpmenu}</div>
	<div id="sort">[{reviews} - {numreviews}] &nbsp; {printicon} {toc}<br />
	<div align="right">{adminlinks}</div><br />
	{textsizer}</div>
	
	<!-- START BLOCK : storynotes -->
	<blockquote><h2>Story notes:</h2> {storynotes}</blockquote>
	<!-- END BLOCK : storynotes -->
	
	<!-- START BLOCK : notes -->
	<blockquote><b>Chapter notes:</b> {notes}</blockquote>
	<!-- END BLOCK : notes -->
	
	<div id="story">{story}</div>
	
	<!-- START BLOCK : endnotes -->
	<blockquote><b>Chapter end notes:</b> {endnotes}</blockquote>
	<!-- END BLOCK : endnotes -->
	
	<div id="pagelinks">
		<div id="prev">{prev}</div>
		<div class="jumpmenu">{jumpmenu2}</div>
		<div id="next">{next}</div>
	</div>
	<br /><br />
	<div class="respond">{roundrobin}</div>
	<div id="reviewform">{reviewform}</div>
</div>
<!-- INCLUDE BLOCK : footer -->